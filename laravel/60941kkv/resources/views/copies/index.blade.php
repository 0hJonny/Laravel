...
<div class="d-flex justify-content-between align-items-center">
    ...
    <form method="GET" action="{{ route('copies.index') }}" class="form-inline mb-3 float-right">
...
<a href="{{ route('copies.create') }}" class="btn btn-primary mb-3">Добавить новый экземпляр</a>
                ...
                <td>
                    <div class="dropdown">
                        ...
                        <div class="dropdown-content">
                            ...
                            <a href="{{ route('copies.duplicate', $copy->copy_id) }}">Копировать</a>
                            <a href="{{ route('copies.show', $copy->copy_id) }}">Просмотр</a>
                            <form action="{{ route('copies.destroy', $copy->copy_id) }}" method="POST">
                                @csrf
                                @method('DELETE')
...
@if ($copies->lastPage() > 1)
    <div class="pagination">
        @if ($copies->onFirstPage())
            <span class="page-item disabled">← Назад</span>
        @else
            <a href="{{ $copies->previousPageUrl() }}" class="page-item">← Назад</a>
        @endif
        ...
        @if ($copies->hasMorePages())
            <a href="{{ $copies->nextPageUrl() }}" class="page-item">Вперед →</a>
        @else
